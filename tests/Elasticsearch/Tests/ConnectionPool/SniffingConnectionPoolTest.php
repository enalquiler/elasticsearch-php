...
use Enalquiler\Elasticsearch\Common\Exceptions\NoNodesAvailableException;
use Enalquiler\Elasticsearch\ConnectionPool\SniffingConnectionPool;
use Mockery as m;
...
class SniffingConnectionPoolTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $mockConnection = m::mock('\Enalquiler\Elasticsearch\Connections\Connection')
                          ->shouldReceive('ping')
                          ->andReturn(true)
                          ->getMock()
        ...
        $selector = m::mock('\Enalquiler\Elasticsearch\ConnectionPool\Selectors\RoundRobinSelector')
                    ->shouldReceive('select')
                    ->andReturn($connections[0])
                    ->getMock();

        $connectionFactory = m::mock('\Enalquiler\Elasticsearch\Connections\ConnectionFactory');
    ...
    public function testAddOneHostAndTriggerSniff()
    {
        ...
        $connections = array($mockConnection);
        $mockNewConnection = m::mock('\Enalquiler\Elasticsearch\Connections\Connection')
                             ->shouldReceive('isAlive')->andReturn(true)->getMock();
    ...
    /**
     * @expectedException Enalquiler\Elasticsearch\Common\Exceptions\NoNodesAvailableException
     */
    ...
    public function testAddOneHostSniffTwo()
    {
        ...
        $newConnections[] = m::mock('\Enalquiler\Elasticsearch\Connections\Connection')
                             ->shouldReceive('isAlive')->andReturn(true)->getMock();
    ...
    public function testTen_TimeoutNine_SniffTenth_AddTwoAlive()
    {
        ...
        foreach (range(1, 10) as $index) {
            $mockConnection = m::mock('\Enalquiler\Elasticsearch\Connections\Connection')
                              ->shouldReceive('ping')->andReturn(false)->getMock()
                              ->shouldReceive('isAlive')->andReturn(true)->getMock()
                              ->shouldReceive('sniff')->andThrow('Enalquiler\Elasticsearch\Common\Exceptions\Curl\OperationTimeoutException')->getMock();
